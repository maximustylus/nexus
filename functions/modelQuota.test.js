...
describe('nextUsable — the best model that has not refused', () => {
    ...
    it('skips the demoted head — the exact free-tier 2.5-pro case, by position', () => {
        const d = createDemotions();
        d.demote('models/' + MODEL_PRIORITY[0], NOW);
        expect(nextUsable(d, NOW)).toBe('models/' + MODEL_PRIORITY[1]);
    });
