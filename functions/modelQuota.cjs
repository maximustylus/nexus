...
/**
 * The list and the fallback live in `modelAvailability.cjs` — the other half of
 * `AU30` — and are re-exported here so nothing that imported them from this
 * module breaks. There used to be a second copy in this file. It named three
 * models Google had withdrawn and a fallback that no longer existed, and nothing
 * noticed because nothing compared the two. One list, one owner.
 */
const { MODEL_PRIORITY, SAFE_FALLBACK_MODEL } = require('./modelAvailability.cjs');
