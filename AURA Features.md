...
She does not just read data; she actively interprets **cognitive load**, negotiates **scheduling conflicts**, executes **database mutations**, and mitigates **burnout risk** acting as a proactive middleware for the entire clinical team.
...
AURA serves as a real-time calendar mediator. She actively listens to the database for peer-to-peer shift swap requests. 
* **Proactive Alerts:** If a colleague requests coverage, AURA forcefully opens her UI and delivers a high-priority `ROSTER_ALERT` with the swap details and rationale.
...
AURA is self-aware and dynamically shape-shifts her UI and persona based on the user's immediate operational or emotional needs:
...
Bypassing mobile browser constraints, AURA can compile her generated text (like policies or SOPs) into true Microsoft Word `.docx` Blob objects. With a single click of the `[Export .DOC]` button, she triggers a native file download directly to iOS, Android, or macOS devices.
...
AURA doesn't wait for staff to burn out; she actively reaches out.
...
AURA is fully aware of her environment. 
...
* **Demo Sandbox:** Seamlessly shifts her identity to manage a simulated "Marvel Superhero CEP Team," allowing future leaders to practice resolving rostering conflicts and generating simulated AI audits without exposing real hospital data.
...
3. **Strictly NO PHI (PDPA Compliance):** AURA tracks *operational* data, not clinical charts. She does not have EMR access. **Do not type patient names or NRICs.** Use placeholders exclusively (e.g., `[Patient]`, `[Clinician]`).
