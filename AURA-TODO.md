...
| `AU33` | **A rework's change summary is false — and the first prompt line did not fix it.** P8.8 turn 8. Run 1: *"only the header changed"* while Handover and Scope were dropped. **Re-opened 2026-09-05 after runs 4 and 5**: with MODE 2 rule 4 in place the reply now LISTS changes properly (header, intro, title block) and then says *"I kept the core procedural steps exactly the same"* over a seven-section, ~18-step SOP condensed to four one-liners, 1,863 chars to 639. The prompt line shaped the form of the sentence and left the falsehood in it — a request, ignored twice. Two responses. (1) The rule now says the specific thing it lacked: do not shorten, merge, condense or summarise; if you did, say so and never say "the same". (2) **The harness measures it**: `reworkNotRewrite` (size ratio ≥ 0.6 and ≥ 50% of the previous document's lines carried) and `changeClaimHonest` (a "kept the same" claim over a document under 70% of the previous size fails). Run 5's SOP and memo are the fixtures. **Cloud runs 1 and 2 (2026-09-05, the first runs of `verify-aura.yml`)**: run 1 `size 1.10, 0% carried` (same length, nothing verbatim); run 2 `size 0.48, 7% carried` AND the reply said *"I kept the procedural steps exactly the same"* over a 1,181-char SOP condensed to a 565-char memo, and claimed to have *"removed the SOP title and document control block"* — there was no document control block. **Both detectors fired, correctly.** That is four runs out of four since the rule was written in which the model condensed on *"make it a memo"* and asserted sameness: the request is not being honoured, and the harness now proves it every run instead of a person having to notice. The only CONTROL available is app-side — the server appending a true sentence to the reply (*"this document is 48% the length of the previous one"*) when the user asked for a change-only edit and the document shrank — and that is a product decision (the application speaking inside AURA's reply) for the owner, not a fix to make unasked. Stays OPEN; the verdict is the owner's. | low · **`OPEN`** |
