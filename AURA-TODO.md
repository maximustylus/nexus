...
| `OPEN`, mine | **0 engineering; 2 awaiting the owner's decision (`AU31`, `AU32`)** | The engineering queue is genuinely empty as of 2026-08-24: `AU3`'s rules backstop and `AC16`'s latch — the two residuals the previous version of this row disclosed — are closed above, both with evidence. What remains is the owner's column and the standing verification work (`P8.8`, `CD13`). |
...
| Id | Finding | Severity |
|---|---|---|
| `AU31` | **MODE 3 re-proposes the previous turn's figures when the user gives none.** P8.8 turn 11, **stable across three live runs, byte-identical**: after *"Log 35 patients for January"* (turn 10, correctly carded), the bare *"Log my workload."* returns a FILLED card — `target_value: 35, target_month: 0` — and the reply *"I am proposing to log 35 patients for January against your personal workload"*. The sheet expected a question for the number and period, and every `db_workload` field null. The model inherited rather than invented, which is the defence, and it is not a strong one: the user said nothing about a figure, and the app renders a ready-to-approve card for one. Nothing is written without a click (`AC16`), so this is a mis-click hazard in a clinical workload record, not a silent write. Options, in the owner's gift: harden the MODE 3 instruction so a proposal requires a figure IN THE CURRENT TURN; or accept context-carrying and make the card say *"carried from your previous message"*. Not a `P3` violation strictly (nothing was made up) — a `P2` one (the acceptance criterion was invented and met). | medium · **`OPEN`** |
| `AU32` | **MODE 1 sometimes claims the write happened.** P8.8 turn 4 (*"Thanks, that helped."*), **2 of 3 live runs**: *"I have noted your energy levels for today to help us keep track."* — a completion claim, the `P7` class. Two honest caveats. First, it may be TRUE: MODE 1's assessment is written to `wellbeing_history` by the client after the turn, so the clinician's record does get the entry. `P7` is about asserting completion the model cannot itself confirm, and whether that sentence misleads is the owner's call, not the harness's. Second, the harness missed it on run 1 — `noClaims` was not on turn 4, and `CLAIMED_ACTION` had no *noted* — both fixed in `6d31dd8`. Same turn, both runs where it appeared, also emitted a full assessment `action` document in reply to a thank-you; run 2's turn 9 rendered a chat summary as a document. If MODE 1/2 reach for `action` too readily, clinicians get export cards they did not ask for. Judgement, not a rule breach. | low · **`OPEN`** |
| `AU30` | ~~**Every Gemini model AURA could reach was dead, and discovery reported success.**~~ **CLOSED 2026-09-05 (`aura`).** `resolveModel()` treated presence in `GET /v1beta/models` as permission to call a model. It is not: a key issued today LISTS `gemini-2.5-pro` and is then refused at `:generateContent` — *"no longer available to new users"* — so discovery logged a resolved model and the call failed, surfacing to the clinician as a broken assistant rather than a model problem. Underneath that, the list had rotted: `gemini-2.0-flash`, `gemini-1.5-pro` and `gemini-1.5-flash` no longer appear in the model list AT ALL, and `gemini-1.5-flash` was ALSO `SAFE_FALLBACK_MODEL` — so on a new key every path led to a 404. Latent only because production runs on a grandfathered key; **rotating `GEMINI_API_KEY` would have taken AURA down for every user**, which is how it was found — the P8.8 runner hit it first with a fresh key. Fix: `functions/modelAvailability.cjs` holds the candidate order, the fallback and `classifyProbe()`; `resolveModel()` now returns a model only once it has ANSWERED a real generation. Ambiguity resolves to `'unknown'`, never `'no'` — a wrong demotion costs the best model for a warm container's whole life (hours), a wrong `'unknown'` costs one re-check. The fallback must now itself be a candidate, so it can never again be a name nothing checks. **`modelAvailability.test.js` — 32 passed.** The runner imports the same module, so harness and deployment cannot disagree about which model AURA runs on. | high · **`DONE`** |
