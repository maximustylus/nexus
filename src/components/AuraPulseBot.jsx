...
export default function AuraPulseBot({ isOpen, onClose, onOpen, user }) {
    ...
    const exportToICS = useCallback((text) => {
        ...
        // Matches typical dates like "16 Apr 2026", "01 May 2026", etc.
        ...
        lines.forEach(line => {
            ...
            if (match) {
                ...
                if (!isNaN(parsedDate.getTime())) {
                    // Clean up the string to find the milestone name
                    let description = line
                        .replace(/\|/g, '')        // Remove markdown table pipes
                        .replace(dateStr, '')      // Remove the date string itself
                        .replace(/^[,\s\-]+/, '')  // Remove leading commas/spaces
                        .trim();
                        
                    let title = description.split(',')[0]; // If CSV style, grab the first chunk
        ...
        events.forEach(event => {
            ...
            // Adds a 48 hour reminder automatically to the Outlook event
